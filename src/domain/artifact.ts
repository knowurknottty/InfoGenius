...
export const DatasetQualityFindingSchema = z.object({ code: z.string().min(1), severity: z.enum(["info", "warning", "error"]), columnId: z.string().optional(), message: z.string().min(1) });
export const DatasetRecordSchema = z.object({
  ...
  representedRowCount: z.number().int().nonnegative(),
  missingness: z.record(z.string(), z.number().min(0).max(1)),
  storage: z.object({ strategy: z.enum(["inline", "indexeddb", "external_ref", "derived"]), reference: z.string().optional() }),
  warnings: z.array(z.string()).optional(),
  qualityFindings: z.array(DatasetQualityFindingSchema).optional(),
